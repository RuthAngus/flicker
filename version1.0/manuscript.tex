...
\begin{equation}
	\log_{10}(F_8) \sim \mathcal{N} \left(\mu = \alpha_\rho +
    \beta_\rho \log_{10}(\rho_\star), \sigma = \sqrt{\sigma_{\rho}^2 +
...
written as
\begin{eqnarray}
	& p(F_8|\rho_\star, \alpha, \beta, \sigma_{\rho}) =  \\ \nonumber
						      & \exp \left[-\frac{1}{2}
...
We used the uninformative prior for the parameters of a straight line with
unknown variance, outlined in \citet{vanderplas},
\begin{equation}
...
The solid black line shows the model with the parameter values quoted in the
text.
...
\begin{table}
\begin{center}
\caption{Median parameter values with 16th and 84th
