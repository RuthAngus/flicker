...
from noisy_plane import generate_samples, lnlikeHFM
...
def lnprior(pars, mm=False):
    if -10 < pars[0] < 10 and -10 < pars[1] < 10 and -10 < pars[2] < 10 \
            and -10 < pars[3] < 10 and -10 < pars[4] < 10 and 0 < pars[5] < 1:
        return 0.
    return -np.inf

def lnprob(pars, samples, obs, u, mm=False):
    return lnlikeHFM(pars, samples, obs, u) + lnprior(pars)

def MCMC(whichx, nsamp, fname, nd, bigdata, burnin=500, run=500):

    rho_pars = [-2., 6., .0065, 3., 1., .1]
    logg_pars = [-1.850, 7., .0065, 3., 1., .1]
    pars_init = logg_pars
    if whichx == "rho":
        pars_init = rho_pars
